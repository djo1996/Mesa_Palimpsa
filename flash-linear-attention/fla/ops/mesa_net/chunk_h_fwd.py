...
def chunk_mesa_net_fwd_kernel_h(
    ...
    if USE_INITIAL_STATE:
        p_h0 = tl.make_block_ptr(h_init + i_nh * K*K, (K, K), (K, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0))
    ...
    for i_t in range(NT):
        ...
        p_k2 = tl.make_block_ptr(k + (bos*H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
        p_v = tl.make_block_ptr(v + (bos*H + i_h) * V, (T, V), (H*V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
        p_beta = tl.make_block_ptr(beta + (bos*H + i_h), (T,), (H,), (i_t * BT,), (BT, ), (0,))
        b_beta = tl.load(p_beta, boundary_check=(0,))

        o_kk = ((boh + i_s) * H + i_h).to(tl.int64) * K*K
        o_kv = ((boh + i_s) * H + i_h).to(tl.int64) * K*V
        p_h = tl.make_block_ptr(h + o_kk, (K, K), (K, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0))
        p_h_kv = tl.make_block_ptr(h_kv + o_kv, (K, V), (V, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0))
    ...
    if STORE_FINAL_STATE:
        p_ht = tl.make_block_ptr(h_final + i_nh * K*K, (K, K), (K, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0))
...
def chunk_mesa_fwd_h(
    ...
    h = k.new_empty(B, NS, H, K, K, dtype=k.dtype if not states_in_fp32 else torch.float)
    h_kv = k.new_empty(B, NS, H, K, V, dtype=k.dtype if not states_in_fp32 else torch.float)
    h_final = k.new_empty(N, H, K, K, dtype=torch.float) if output_final_state else None
