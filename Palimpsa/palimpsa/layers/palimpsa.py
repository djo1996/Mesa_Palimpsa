...
class Palimpsa(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        output_final_state = use_cache or (self.eval_diagnosis and not self.training)
        if not self.metaplasticity:
            g_log = -dt * A 
            if mode == 'chunk':
                outputs = chunk_simple_gla(q=q, k=k, v=v, g=g_log, initial_state=recurrent_state, output_final_state=output_final_state, cu_seqlens=cu_seqlens)
            else:
                outputs = fused_recurrent_simple_gla(q=q, k=k, v=v, g=g_log, initial_state=recurrent_state, output_final_state=output_final_state, cu_seqlens=cu_seqlens)
            ...
            if mode == 'chunk':
                outputs = chunk_palimpsa(q=q, k=k, v=v, b=b, gt=dt, g=A, Ip=Ip, output_final_state=output_final_state, cu_seqlens=cu_seqlens, chunk_size=16, initial_mu_state=active_mu, initial_I_state=active_I)
                if use_cache:
                ...
                outputs = fused_recurrent_palimpsa(q=q, k=k, v=v, b=b, gt=dt, g=A, Ip=Ip, initial_mu_state=active_mu, initial_I_state=active_I, output_final_state=output_final_state, cu_seqlens=cu_seqlens)
                if use_cache:
                    o, final_mu, final_I = outputs
                    recurrent_state = (final_mu, final_I)
                else:
